...
CFLAGS_LINK_TO_LIBECC_OPTIMIZED := \
		-fno-builtin -fno-builtin-printf -DWORDSIZE=64 -DWITH_STDLIB -DWITH_CKB \
...
$(SECP256K1_SRC_20210801):
	cd deps/secp256k1-20210801 && \
		./autogen.sh && \
		CC=$(CC) LD=$(LD) ./configure --with-bignum=no --host=$(TARGET) \
...
deps/mbedtls/library/libmbedcrypto.a:
	...
	make -C deps/mbedtls/library \
		APPLE_BUILD=0 AR=$(AR) CC=${CC} LD=${LD} CFLAGS="${PASSED_MBEDTLS_CFLAGS}" LDFLAGS="${LDFLAGS}" \
		libmbedcrypto.a
...
build/auth: \
		...
		build/libed25519.a \
		build/libnanocbor.a
	$(CC) $(AUTH_CFLAGS) $(LDFLAGS) -pie -Wl,--dynamic-list c/auth.syms -o $@ $^
	cp $@ $@.debug
	$(OBJCOPY) --strip-debug --strip-all $@
	ls -l $@

build/auth_libecc: c/auth_libecc.c $(LIBECC_OPTIMIZED_FILES)
	$(CC) $(AUTH_CFLAGS) $(CFLAGS_LINK_TO_LIBECC_OPTIMIZED) $(LDFLAGS) \
		-pie -Wl,--dynamic-list c/auth.syms -o $@ $^
	cp $@ $@.debug
	$(OBJCOPY) --strip-debug --strip-all $@
	ls -l $@
