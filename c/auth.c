...
static int monero_public_key_is_small_order(const uint8_t public_key[32]) {
    static const uint8_t small_order_keys[][32] = {
    ...
    for (size_t i = 0;
         i < sizeof(small_order_keys) / sizeof(small_order_keys[0]); i++) {
        if (memcmp(public_key, small_order_keys[i], MONERO_PUBKEY_SIZE) == 0) {
            return 1;
        }
    }
    return 0;
...
int ed25519_verify_monero(const unsigned char *signature,
    ...
    if (sc_check(sig_c) != 0 || sc_check(sig_r) != 0 || !sc_isnonzero(sig_c) ||
        monero_public_key_is_small_order(public_key)) {
        return 0;
    }
